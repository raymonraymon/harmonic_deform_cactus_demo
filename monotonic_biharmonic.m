function W = monotonic_biharmonic(varargin)
  ...
  if(size(F,2)==4)
    ...
    assert(strcmp('opt_type','legacy-quad'));
  else
  ...
  if ~isempty(S)
    ...
    case 'legacy-quad'
      ...
      Q = Q+2*diag(sparse(data_weight));
    case {'quad','conic'}
      FF = [FF;diag(sqrt(2)*sparse(sqrt(data_weight)))];
    end
    l = bsxfun(@plus,l,-2*bsxfun(@times,data_weight,S));
  end

  if isempty(A)
    % initial monotonic guess 
    WH = oneharmonic(V,F,b,bc);
    A = cell(m,1);
    % loop over handles
    for i = 1:m
      A{i} = monotonicity_matrix(WH(:,i),F,'OneInOneOut',true);
    end
  elseif isnumeric(A)
    A = {A};
  end
    ...
    while(iter <= max_iter)
      ...
      case 'interior-point'
        ...
        case 'legacy-quad'
          ...
          [x,fval,err] = quadprog(Q,l(:,i),Aleq,bleq,Aeq,bc(:,i),lx,ux,[],param);
          if(err ~= 1)
            fprintf([...
              '----------------------------------------------------------\n' ...
